...
def send_haptic_feedback(note, intensity, note_duration):
    for no_octave in range(1, 8):
        octave_key = f"oct{no_octave}"
        if octave_key in note_to_actuator and note in note_to_actuator[octave_key]:
            no_octave = note // 12 - 1
            ...
            dot_frame = {
                ...
                "DurationMillis": 1000  # Fixed duration for now
            }
...
def handle_midi_message(message):
    ...
    if message.type == 'note_on':
        ...
        if velocity > 0:
            ...
            send_haptic_feedback(note, velocity, 'crotchet')
        else:
